...
void mpu6050_init(void) {
    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1,
                   0x00);  // exit sleep mode
    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_CONFIG,
                   0x01);  // LPF, bandwidth = 184(accel) and 188(gyro)
    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_GYRO_CONFIG,
                   1 << 4);  // gyro ADC scale: 1000 deg/s
    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_ACCEL_CONFIG,
                   0x00);  // accel ADC scale: 2 g

    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_INT_ENABLE,
                   0x00);  // enable data ready interrupt
    i2c_write_byte(MPU6050_ADDRESS, MPU6050_RA_SIGNAL_PATH_RESET,
                   0x00);  // don't reset signal path
}
